import numpy as np
import matplotlib.pyplot as plt
from wasserstein import wasserstein_1d


def gaussian(x, mu, sigma):
	return np.exp(- ((x - mu) / sigma) ** 2 / 2)

# def gaussian_mix(x, mu, sigma, w1=0.5, w2=0.5):
# 	return w1 * gaussian(x, mu, sigma) + w2 * gaussian(x, - mu, sigma)

# def gaussian_dx(x, mu, sigma):
# 	return - (x - mu) / sigma ** 2 * gaussian(x, mu, sigma)

# def gaussian_mix_dx(x, mu, sigma, w1=0.5, w2=0.5):
# 	return w1 * gaussian_dx(x, mu, sigma) + w2 * gaussian_dx(x, - mu, sigma)

# def V_dx(x, mu, sigma):
# 	return - gaussian_dx(x, mu, sigma) / gaussian(x, mu, sigma)

# def V_dx_mix(x, mu, sigma, w1=0.5, w2=0.5):
# 	return - gaussian_mix_dx(x, mu, sigma, w1, w2) / gaussian_mix(x, mu, sigma, w1, w2)


def V_linear(x, gamma=1.0):
	return gamma * x ** 2

def V_linear_dx(x, gamma=1.0):
	return 2 * gamma * x


def high_order_runge_kutta(x, sigma=1.0, gamma=1.0, h=1e-1, T=100):

	x_list = []
	
	f = lambda x: - V_linear_dx(x, gamma)

	# sigma = 0.
	# f = lambda x: - V_linear_dx(x)

	def runge_kutta_step(x, t, h):
		# draw random csi
		csi = np.random.normal(0, 1, x.shape)
		y1 = x + np.sqrt(2 * h) * sigma * csi
		y2 = x - 3/8 * h * f(y1) + np.sqrt(2 * h) * sigma * csi / 4
		x = x - 1/3 * h * f(y1) + 4/3 * h * f(y2) + sigma * np.sqrt(h) * csi
		return x
	
	# def runge_kutta_step(x, t, h):
	# 	csi = jax.random.normal(jax.random.key(t * 100), (x.shape[0],))
	# 	y1 = x + sigma * np.sqrt(h) * csi
	# 	y2 = x - h/2 * f(y1) + sigma/2 * np.sqrt(h) * csi
	# 	y3 = x + 3 * h * f(y1) - 2 * h * f(y2) + sigma * np.sqrt(h) * csi
	# 	x = x - 3/2 * h * f(y1) + 2 * h * f(y2) + h/2 * f(y3) + sigma * np.sqrt(h) * csi
	# 	return x

	for t in range(T):
		if t > 0 and t % 1000 == 0:
			print(f'Iter: {t}')
		x_list.append(x)
		x = runge_kutta_step(x, t, h)
		# x = runge_kutta_step_2(x, h)
	
	return x, x_list


# def high_order_runge_kutta_vec(x, alpha=5.0, h=1e-1, T=100):

# 	# x_list = []
	
# 	sigma = np.sqrt(2 * alpha)
# 	# sigma = 0.
# 	# V_tilde = lambda x: alpha * V_dx(x, 0, 1)
# 	V_tilde = lambda x: alpha * V_dx_mix(x, 3, 1, 0.3, 0.7)
# 	f = lambda x: - V_tilde(x)

# 	# sigma = 0.
# 	# f = lambda x: - V_linear_dx(x)

# 	def runge_kutta_step(x, key):
# 		csi = jax.random.normal(key, (x.shape[0],))
# 		y1 = x + np.sqrt(2 * h) * sigma * csi
# 		y2 = x - 3/8 * h * f(y1) + np.sqrt(2 * h) * sigma * csi / 4
# 		x = x - 1/3 * h * f(y1) + 4/3 * h * f(y2) + sigma * np.sqrt(h) * csi
# 		return x, key
	
# 	def body_fn(i, state):
# 		x, key = state
# 		x, key = runge_kutta_step(x, key)
# 		return (x, key)

# 	x, _ = jax.lax.fori_loop(0, T, body_fn, (x, jax.random.key(0)))
	
# 	return x # x_list


def main():

	# set numpy seed
	np.random.seed(0)
	
	T = 10 ** 5 # time steps
	h = 0.01 # discretization step
	N = 10 # number of samples
	# x0 = jax.random.normal(jax.random.key(0), (N,)) - 5
	x0 = np.zeros(N)
	sigma = 1.0 # FIXED
	gamma = 1.0 # FIXED

	x, x_list = high_order_runge_kutta(x0, sigma=sigma, gamma=gamma, h=h, T=T)
	# x = high_order_runge_kutta_vec(x0, T=T)

	# Wasserstein distance evolution
	# wass_list = []
	# for i in range(1, T):
	# 	wass_list.append(wasserstein_1d(x_list[i], x_list[i-1], p=2))
	# plt.plot(np.cumsum(np.array(wass_list)) / (np.arange(T - 1) + 1))
	# plt.show()

	x_plot = np.linspace(-10, 10, 1000)
	# y = gaussian_mix(x_plot, 3, 1, 0.3, 0.7)
	y = gaussian(x_plot, 0, 1)
	y /= y.sum()
	plt.plot(x_plot, y, label='True')
	plt.plot(x0, np.zeros_like(x0), 'ro', label='Initial')
	plt.plot(x, np.zeros_like(x), 'bo', label='Final')
	plt.legend()
	plt.show()

	# x_plot = np.linspace(-10, 10, 1000)
	# fig, ax = plt.subplots()
	# for idx, x in zip(range(T), x_list):
	# 	ax.plot(x_plot, gaussian(x_plot, 0, 1), label='PDF')
	# 	# ax.plot(x_plot, gaussian_mix(x_plot, 3, 1, 0.3, 0.7), label='PDF')
	# 	ax.scatter(x, np.zeros(N), alpha=0.1)
	# 	plt.title('iter: {}'.format(idx))
	# 	plt.legend()
	# 	plt.ion()
	# 	plt.draw()
	# 	plt.show()
	# 	plt.pause(0.01)
	# 	ax.clear()

	# Compute error for the second moment
	emp_var = np.zeros(N)
	for x in x_list:
		emp_var = emp_var + x ** 2
	emp_var = emp_var / T
	var = 0.5 # true variance
	err = np.abs(emp_var - var)
	print(err)

	# Compute error for the first moment
	emp_mean = np.zeros(N)
	for x in x_list:
		emp_mean = emp_mean + x
	emp_mean = emp_mean / T
	mean = 0 # true mean
	err = np.abs(emp_mean - mean)
	print(err)


if __name__ == '__main__':
	
	main()

	# errors with T = 10 ** 4, N = 10, alpha = 1.0, i.c. = 0
	h_list = [0.1, 0.05, 0.01, 0.005, 0.001]
	err_list = [[1.0089527, 1.0216413, 1.000571,  1.006411,  1.0497797, 0.9595082, 0.9905524, 0.9434501, 1.0467663, 1.1198545],
				[1.0184371, 1.0184956, 1.004668,  1.0222466, 1.0531402, 0.9669093, 0.9913651, 0.9004512, 1.0636544, 1.141215 ],
				[1.0521693, 1.1700025, 1.0162065, 1.0715837, 1.0593469, 0.9698435, 1.0935578, 0.8962941, 1.1572883, 1.1571603]
				]
	# avg_err_list = [sum(errs) for errs in err_list]
	# plt.loglog(h_list, avg_err_list, label='err')
	# plt.loglog(h_list, [(h / h_list[0]) ** 2 for h in h_list], label='h^2')
	# plt.legend()
	# plt.show()

	# errors with T = 10 ** 5, N = 10, alpha = 0.1, i.c. = 0
	h_list = [0.1, 0.01, 0.001]
	err_list = [[0.80821972, 0.79701693, 0.79474175, 0.79288159, 0.78036592, 0.83198506, 0.80930997, 0.8023197,  0.79975426, 0.85861565],
			 	[0.5561843,  0.45027855, 0.44697664, 0.63506826, 0.65474451, 0.66155746, 1.44106652, 0.46151919, 0.34402352, 1.1211351 ],
				[0.43429306, 0.03803745, 0.00548279, 0.11353126, 0.32287249, 0.01839569, 0.2756504,  0.02074572, 0.08902768, 0.92224576]]
	# avg_err_list = [sum(errs) for errs in err_list]
	# plt.loglog(h_list, avg_err_list, label='err')
	# plt.loglog(h_list, [(h / h_list[0]) ** 2 for h in h_list], label='h^2')
	# plt.legend()
	# plt.show()

	h_list = [0.1, 0.01, 0.001]
	err_list = [
		[0.25851862, 0.26218945, 0.25603403, 0.2601534,  0.25698842, 0.26107439, 0.26018294, 0.25930653, 0.25901014, 0.25533727]
	]